...
reuna.canvas.init = function(canvas_id, drawCallback) {
    ...
    if (reuna.canvas._canvas.getContext) {
        ...
        window.addEventListener('mousemove',    reuna.canvas._mouseMoveRouter, false);
...
reuna.canvas._mousePressedRouter    = function(e) { reuna.canvas._mouseEventXYBtnRouter(e, reuna.canvas.mousePressed); }
reuna.canvas._mouseMoveRouter       = function(e) { reuna.canvas._mouseEventXYBtnRouter(e, reuna.canvas.mouseMoved); }

/* extracts the parameters and calls a mouse event callback */
reuna.canvas._mouseEventXYBtnRouter = function(e, callback) {
	...
	*/
    ...
    // echo("mouse event at "+x+","+y);
    
    if(callback) { callback(x, y, btn); }
}
